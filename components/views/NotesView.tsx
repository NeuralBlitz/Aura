...
const NotesView: React.FC = () => {
  ...
  return (
    <ModuleLayout title="Neural Codex" subtitle="Encrypted Knowledge Base" status="SYNCED" icon={BookOpen} color="cyan">
      <div className="flex h-full bg-black/20 rounded-[2rem] border border-white/10 overflow-hidden shadow-2xl relative w-full">
        ...
        <div className={`w-72 bg-black/40 backdrop-blur-xl border-r border-white/5 flex-col shrink-0 transition-transform duration-300 z-50 ${isMobileMenuOpen ? 'absolute inset-y-0 left-0 translate-x-0 flex shadow-2xl' : 'absolute inset-y-0 left-0 -translate-x-full md:static md:translate-x-0 md:flex'}`}>
        ...
        {/* === MIDDLE: INDEX LIST === */}
        <div className={`w-full md:w-80 lg:w-96 bg-black/20 border-r border-white/5 flex flex-col ${activeNoteId ? 'hidden md:flex' : 'flex'}`}>
          <div className="p-6 border-b border-white/5 space-y-4 shrink-0">
            <div className="relative group">
              ...
              <input type="text" value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} placeholder="Search index..." className="w-full bg-black/50 border border-white/10 rounded-2xl py-3.5 pl-10 pr-4 text-xs font-bold text-white focus:outline-none focus:border-cyan-500/50 transition-all" />
            </div>
          </div>
          ...
          <div className="flex-1 overflow-y-auto no-scrollbar p-4 space-y-3">
            {filteredNotes.length === 0 ? (
              ...
              filteredNotes.map(note => (
                <button key={note.id} onClick={() => setActiveNoteId(note.id)} className={`w-full text-left p-5 rounded-[1.5rem] border transition-all group relative ${activeNoteId === note.id ? 'metallic-card cyan bg-cyan-900/10' : 'bg-white/[0.02] border-transparent hover:bg-white/[0.04]'}`}>
        ...
        {/* === RIGHT: EDITOR === */}
        {activeNote ? (
          <div className={`flex-1 flex flex-col bg-transparent relative ${!activeNoteId ? 'hidden md:flex' : 'flex z-20 absolute inset-0 md:static'}`}>
          ...
          <div className="flex-1 hidden md:flex flex-col items-center justify-center bg-transparent opacity-30">
