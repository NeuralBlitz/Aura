...
const HealthView: React.FC = () => {
  ...
  const runScan = async () => {
    ...
    try {
      ...
      if (response.ok) {
         ...
         setReports([
            { label: 'Neural Mesh Integrity', status: 'ok', value: '100%' },
            { label: 'Quantum Entanglement', status: 'ok', value: 'Stable' },
            { label: 'Temporal Drift', status: 'warn', value: '0.002ms' },
            { label: 'Substrate Memory', status: 'ok', value: 'Optimal' }
         ]);
      }
    } catch (e) {
      console.error(e);
  ...
  return (
    <ModuleLayout title="System Health" subtitle="Diagnostics & Integrity" status={scanning ? "SCANNING" : "OPTIMAL"} icon={Activity} color="emerald">
      <div className="flex h-full w-full bg-black/20 overflow-hidden font-sans relative">
        <div className="flex-1 overflow-y-auto p-8 no-scrollbar">
           <div className="max-w-3xl mx-auto text-center pt-6 pb-24">
